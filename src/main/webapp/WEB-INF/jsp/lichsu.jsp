			...
			<nav
				...
				<div class="container-fluid">
					...
					<div class="collapse navbar-collapse justify-content-end">
						<sf:form modelAttribute="searchForm" acion="lichsu" method="get" style="display: flex;">
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-header card-header-primary">
									...
									<h4 class="card-title ">Bảng lịch sử mua hàng</h4>
									...
									<div class="table-responsive">
										<table class="table">
											<thead class=" text-primary">
												...
												<th>
													<button xepTheo="customer.name" class="xep">
														Họ và tên
														<c:if test="${searchForm.xepTheo == 'customer.name'}">
															<c:choose>
																<c:when test="${searchForm.thuTu}">&#8593</c:when>
																<c:otherwise>&#8595</c:otherwise>
															</c:choose>
												...
												<th>
													<button xepTheo="customer.phoneNumber" class="xep">
														Số điện thoại
														<c:if test="${searchForm.xepTheo == 'customer.phoneNumber' }">
															<c:choose>
																<c:when test="${searchForm.thuTu}">&#8593</c:when>
																<c:otherwise>&#8595</c:otherwise>
															</c:choose>
												...
												<th>
													<button xepTheo="customer.email" class="xep">
														Mail
														<c:if test="${searchForm.xepTheo == 'customer.email' }">
															<c:choose>
																<c:when test="${searchForm.thuTu}">&#8593</c:when>
																<c:otherwise>&#8595</c:otherwise>
															</c:choose>
											...
											<tbody>
												...
												<c:forEach var="orders" items="${orders}">
													<tr>
														...
														<td style="text-align: center;">${orders.user.user_name}</td>
														<td style="text-align: center;">${orders.user.user_sdt}</td>
														<td style="text-align: center;">${orders.user.user_email}</td>
														<td style="text-align: center;">${orders.getDateFormat()}</td>														
														<td style="text-align: center;">
															<button type="button" class="action_btn" data-toggle="modal"
																data-target="#detail${orders.id}">Chi tiết</button> <!-- Modal -->
															<div class="modal fade" id="detail${orders.id}" tabindex="-1"
																role="dialog" aria-labelledby="#detail${orders.id}"
																aria-hidden="true">
																	...
																	<div class="modal-content">
																		...
																		<div class="modal-body">
																			<div style="background: #ddd; padding: 20px; margin: 20px">
																				...
																				<p>
																					Mã order: <b>${orders.id }</b>
																				</p>
																				<p>
																					Tên người nhận: <b>${orders.user.user_name}</b>
																				</p>
																				<p>
																					Địa chỉ: <b>${orders.address}</b>
																				</p>
																				<p>
																					Số điện thoại: <b>${orders.user.user_sdt}</b>
																				</p>
																				<p>
																					Tổng tiền : <b><fmt:formatNumber type="number"
																							maxFractionDigits="1" value="${orders.total}" /> VND</b>
																				</p>
																			</div>
							
																			<div style="background: #ddd; padding: 20px; margin: 20px">
																				<h3 style="text-align: center;">Sản phẩm bao gồm</h3>
																				<c:forEach var="orderItems" items="${orders.order_items }">
																					<p>
																						- ${orderItems.product.name} : <b><fmt:formatNumber
																								type="number" maxFractionDigits="1"
																								value="${orderItems.price}" /> VND</b> <i>(x${order_items.amount })</i>
																					</p>
																				</c:forEach>
